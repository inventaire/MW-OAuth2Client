...
class OAuth2ClientHooks {
	public static function onSkinTemplateNavigation_Universal( SkinTemplate $skinTemplate, array &$links ) {
		...
		if( $inExt ) {
			$links['user-menu']['anon_oauth_login']['href'] = SkinComponentUtils::makeSpecialUrlSubpage( 'OAuth2Client', 'redirect' );
		} else {
			# Due to bug 32276, if a user does not have read permissions,
			# $this->getTitle() will just give Special:Badtitle, which is
			# not especially useful as a returnto parameter. Use the title
			# from the request instead, if there was one.
			# see SkinTemplate->buildPersonalUrls()
			$links['user-menu']['anon_oauth_login']['href'] = SkinComponentUtils::makeSpecialUrlSubpage(
				'OAuth2Client',
				'redirect',
